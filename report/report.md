...
* `500 Internal Server Error` - отправляется в том случае, если пользователь не в состоянии сделать так, чтобы его корректный запрос выполнился успешно. Т.е. перечисленные выше ошибки (кроме OK) дают ясно понять, что вызвало проблему (в контексте выполнения команды) и почему вина лежит на пользователи, и что пользователь может предпринять (перестать пытаться зайти в `/root`). Но если на серверной стороне удалить файл с базой пользователей, то клиенту легче не станет от того, что прочтет ошибку "550 User Accounts Lost" вместо "500 Internal Server Error".
...
* KILL
    * Forbidden - операция разрешена только суперпользователям; нельзя выполнить KILL для завершения текущего сеанса - для этого есть LOGOUT
...
В связи с тем, что TCP — потоковый протокол, программно невозможно разделить присылаемые данные на сообщения напрямую. Для этого можно использовать несколько подходов. Например, можно заканчивать сообщение определенным символом-разделителем, но тогда считывать из сокета придется побайтно или блоками фиксированной длины. Первое неэффективно, потому что каждое обращение к recv() - это систмный вызов, выполняющийся только ради извлечения одного байта. Второе можно реализовать и это будет приемлимо для чтения небольших данных. Другим вариантом решения вышеназванной проблемы является пересылка перед сообщением его длины.
...
Если длина тела сообщения не нулевая, то добавляется ровно одна пустая строка (`\r\n`), отделяющая заголовок ответа от тела с произвольными данными указанной в заголовке длины. Причем длина всегда задается большей на 2 байта, чем записано реальных данных в тело - учитываются два символа в пустой строке, чтобы клиент мог за один вызов функции [readn()](../tcp/lib/efunc.c) получить все сообщение целиком, не компенсируя два байта. Тогда формат ответа с непустым сообщением выглдит таким образом: `длина_тела статус текстовое_описание\r\n\r\n<сообщение>`

Сообщение может состоять из неопределенного объема данных и произвольного числа строк, поэтому организовать клиенту чтение многострочного тела сообщения не так просто. С другой стороны, можно было бы передавать одну строку, содержащую ответ и закодированный символы перевода строки, но это лишает пользователя возможности пользоваться telnet клиентом. Поэтому было решено, что только первая строка ответа считывается до разделителя, а затем выполняется функция чтения всего тела
...
Клиенты для двух транспортных протоколов полностью абстрагируют детали реализации прикладного протокола: консольный интерфейс у них одинаковый, и команды они выполняют ровно те же самые. В добавок, клиентским приложениям все равно, в каком регистре были введены команды - все они будут преобразованы в верхний регистр. Также стоит отметить, что команды LS, CD и WHO, введенные в клиенте без аргумента, получат аргумент по умолчанию - символ точки. Команде WHO, напомню, без разницы, что передать в качестве аргумента. Похожая ситуация с командой LOGOUT - ей в качестве аргумента задается имя пользователя, и не важно, что пользователь далее вписал что-то другое.
...
```
После примерно двадцати секунд неактивности, сервер "отключил" клиент, освободив структуры данных, связанных с этим клиентов. Клиент затем попытался получить результат команды LS, но у него уже был лишен авторизации.
...
* KILL
    ...
    ```
    ---
    Суперпользователи не могут завершить с помощью KILL собственное текущее соединение, однако могут завершать другие 
    ```
...
* пароль больше 10 символов
    ...
    Unable to log in
    ```
    Аутентификация не пройдена. Можно предположить, что если бы пароль был в пределах первых десяти символов верен, то оставшиеся символы просто были бы проигнорированы. Для проверки был создан новый пользователь с максимально длинным паролем, и опыт был повторен:
    ```
