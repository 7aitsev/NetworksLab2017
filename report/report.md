...
Дальше видим еще один пакет - одиннадцатый - который был повторно отправлен серверу, потому что не пришло подтверждение. Файлы с дампами можно загрузить и просмотреть: [tcp_netem_windows](res/tcp_netem_windows.) и [tcp_netem_linux.pcapng](res/tcp_netem_linux.pcapng).
