...
Этапы создания серверного и клиентского соединения не отличаются в зависимости от платформы, за исключением функии close(), которая в WinSock называется closesocket(), и необходимости на Windows вызывать функцию WSAStartup() перед началом использования сокетов, а также WSACleanup() по завершении работы.
...
Остановим снифферы и посмотрим, что происходило с сетью во время работы приложений:
...
Также попробуем изменить опции netem, чтобы повреждение сегментов было проще обнаружить:
```
...
![Окно Wireshark, когда опции netem были те же, что и при работе с TCP](res/udp_netem_windows.png)
...
![Еще один опыт, как предыдущий](res/udp_netem_linux.png)
...
Системным интерфейсом для любых сетевых взаимодействий является интерфейс сокетов, обеспечивающий удобный доступ к стеку TCP/IP, реализующего коммуникационные протоколы. Сокет абстрагирует сетевой адрес, т.е. за ним скрыто конкретное семейство адресов и способ организации сети. Сокет позволяет управлять транспортом и предоставляет базовые операции ввода-вывода: открытие и закрытие, чтение и запись. Интерфейс сокетов переносимый, поскольку поддерживается многими популярными операционными системами.

На транспортном уровне в IP-сетях есть два основных протокола: TCP и UDP. Необходимо понимать различия между этими двумя протоколами транспортного уровня, поскольку нерациональный выбор может повлечь за собой необоснованные трудности (достаточно представить, каких сил стоило бы разработать telnet на UDP вместо TCP) или практическую невозможность реализации (интернет-телевидение от провайдеров по TCP). Эти протоколы отличаются друг от друга существенно. Самое важное отличие в том, что TCP устанавливает логическое соединение между сторонами обмена и обеспечивает *потоковую передачу* байт, а в UDP все данные отправляются независимо друг от друга с точки зрения протокола.

Разработка сетевого приложения включает в себя этап проектирования прикладного протокола. На прикладной протокол возлагается множество задач - и это значительные упущение стека TCP/IP (до сих пор можно встретить áèòóş êîäèğîâêó). Это видно уже из того, что верхние три уровня модели OSI (прикладной, представления и сеансовый) ложатся в один верхний уровень модели TCP/IP (который называется уровнем приложения или также прикладным). Поэтому помимо формализации набора данных, необходимого для взаимодействия приложений по сети, нужно еще позаботиться о том, как эти данные согласовать между приложениями, работающими на разных платформах. Ведь в задачи уровня представления входит работа с кодировкой текста, сжатие данных, учет порядка байт и прочее. То же относится с теми задачами, которые возлагаются на сеансовый уровень: создание (защищенной) сессии с процедурой входа и ее завершение, синхронизация работы задач, сбор статистики качества обслуживания и прочее.

При разработке сетевых приложений терминального доступа использовалась клиент-серверная архитектура. Клиент и сервер - это программы, выступающие в разных ролях. Сервер обычно прослушивает поступающие данные и обрабатывает их, а клиент является инициатором передачи.

Были написаны вспомогательные функции, используемые при разработке TCP-приложений:
* __readn()__ - учитывает возможность возврата числа байт меньшего, чем было запрошено функцией recv()
* __sendall()__ - является оберткой над send(), которая тоже может вернуть меньшее число байт, чем требовалось отправить
* __readcrlf()__ - читает данные из сокета до перевода строки

TCP-сервер для ОС на Linux подразумевал наличие множества потоков исполнения. Был написан модуль [logger](../tcp/logger/logger.c), который собирает отладочные сообщения от любых потоков, вызывающих функцию `logger_log()`, и выводит их в консоль. Для управления потоками и их синхронизацией использовался POSIX Threads API. Синхронизация потоков необходима, потому что при выполнении некоторых клиентских запросов и команд с консоли управления сервером требовался доступ из разных потоков к структурам с данными о клиентах. Кроме примитивов синхронизации POSIX, применялись функции для атомарного изменения целочисленных значений ([gcc atomic builtins](https://gcc.gnu.org/onlinedocs/gcc/_005f_005fsync-Builtins.html)).

UDP-сервер для Windows обслуживает клиентов в одном потоке, но чтение данных с консоли реализовано в отдельной нити. Управление потоками происходит с помощью Windows API. Для сообщения основному потоку о необходимости обработки команды с консоли сервера используются средства межпроцессного взаимодействия и функции ожидания, предоставляемые программным интерфейсом ОС.

Реализован механизм аутентификации по паре из имени пользователя и пароля. Неавторизованный пользователь может выполнить лишь одну команду протокола AUTH, предназначенную для создания клиентской сессии. На стороне сервера хранится список пользователей, их пароли и уровень привилегии. Только привелигированные пользователи могут выполнять команду KILL.

Клиенты для разных платформ и транспортных протоколов имеют одинаковый пользовательский интерфейс и разработаны согласно типовой структуре сетевого приложения. Клиенты берут на себя задачу распознавания команды и ее аргументов из введенных пользователем данных, а также формирование запроса и ожидание ответа. Сервер дополнительно проверяет команду на соответствие формату прикладного протокола, принимает решение об обработке или отказе обслуживания в зависимости от прав пользователя, формирует ответ.

Разработка всех программ велась на языке C. В отладочных целях использовалась консольная утилита для анализа сетевого трафика `tcpdump`.

В реальной сети происходят сбои, влияющие на доставку информации. Передаваемые пакеты могут теряться, искажаться, дублироваться, приходить на конечный узел в неопределенном порядке. TCP решает часть проблем за счет внутренних механизмов, обеспечивающих надежность: подтверждения и повторная передача, контроль за порядком данных и их неискаженностью (по контрольной сумме). UDP же предоставляет ненадежный сервис, добавляющий поверх IP, по сути, лишь контрольную сумму для защиты собственных заголовков и данных, а также номера портов отправителя и получателя. Сеть, как и для TCP сегментов, сделает все возможное, чтобы доставить IP датаграммы, но в случае с UDP нужно дополнительно реализовывать способы обнаружения повторяющихся данных и их потерь, если такие проблемы не допускаются для корректной работы приложений.

UDP-приложения используют протокол, в котором каждый ответ и запрос имеет уникальный номер в рамках клиентской сессии. Это позволяет обнаружить дублирование пакетов, в некоторых случаях - их задержку или потерю. С целью выявления потери связи с одной из сторон обмена был реализован механизм пульсации. Это также позволяет клиенту вовремя сообщить о потере запроса или ответа и перейти к ожиданию новой команды от пользователя.
