...
Дисциплина *netem* умеет эмулировать потери, дублирование, искажение и перемешивание пакетов, а также добавлять задержки с различной функцией распределения и ограничивать пропускную способность (см [man tc-netem(8)](https://www.linux.org/docs/man8/tc-netem.html), а также [первую ссылку в Google](https://wiki.linuxfoundation.org/networking/netem) по запросу "netem"). Однако __все манипуляции производятся только над исходящим траффиком__ с интерфейся, т.е. с того, что отправляется из ОС на Linux.
...
[terminal] started
```
Запустим tcpdump с фильтром захвата "host 192.168.56.1 and tcp", чтобы оставить только пакеты, в которых содержится указанный IP адрес и содержат TCP заголовки. Вывод сохраним в файл tcp_normal_linux.
```
...
Во время работы по протоколу UDP при анализе сетевого трафика (если не будет сбоев) мы увидем только шесть пакетов, потому что во время работы клиент сделал запрос на аутентификацию (AUTH), на смену директории, чтобы запросить текущую директорию (`CD .`), а затем на завершение сеанса (LOGOUT) - и сервер должен ответить по одному разу. Ни установления соединения, ни его разрыва не требуется, также, как нет датаграмм с подтверждением получения.
